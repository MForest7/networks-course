...
#### Решение
Время с момента создания бита до момента его декодирования можно разделить на 2 этапа:

* Формирование пакета $-$ $\frac{56 \cdot 8}{128000} = 0.0035\,с$
* Распространение пакета $-$ $0.005 + \frac{56 \cdot 8}{1000000} = 0.005448$

Таким образом, искомое время $-$ $0.0035+0.005448=0.008948\,с$
...
#### Решение
Средняя общая задержка составляет $0.01 + \frac{1}{100} = 0.02\,с$.
Так как в буфере в среднем содержится $10$ пакетов, по формуле Литтла $a = \frac{N}{d} = \frac{11}{0.02} = 550$ пакетов в секунду.
...
#### Решение
1. Так как непосредственно передача пакетов занимает одинаковое время, и узкое место $-$ первая линия, то передача второго пакета начнётся (а значит, и завершится) спустя $\frac{L}{R_S}$ времени от начала (завершения) передачи первого пакета по первой линии.
2. В этом случае, так как разница во времени передачи первого и второго пакетов составит $\frac{L}{R_S} < \frac{L}{R_C}$ $-$ времени передачи первого пакета по второму каналу. Значит, второй пакет будет в очереди $\frac{L}{R_C} - \frac{L}{R_S}$ секунд. Тогда, чтобы очередь была нулевая, нужно отправить пакет с задержкой не менее $T = \frac{L}{R_C} - \frac{L}{R_S}$ секунд.
...
#### Решение
1. $\Delta = \frac{850000}{15 \cdot 10^6} = 0.0566\,с$
2. Среднее время ответа $-$ сумма задержки в локальной сети ($\frac{850000}{10^8} = 0.0085\,с$), задержки ответа и задержки доступа, которая составляет $\frac{\Delta}{1 - \Delta B} = \frac{0.0566}{1 - 16 * 0.0566} = 0.611\,с$. Таким образом, среднее время ответа составляет $3.612\,с$ 
3. Общее время ответа в таком случае $-$ сумма времени ответа в локальной сети и, с вероятностью $0.4$, времени ответа внешней сети. Время доступа к внешней сети изменится, так как во внешнюю сеть отправляется $0.4 * B = 6.4$ запросов в секунду, и составит $\frac{0.0566}{1 - 6.4 * 0.0566} = 0.0887\,с$. Таким образом, общее время задержки ответа составит $0.0085 + 0.4 * (3 + 0.0887) = 1.2439\,с$
