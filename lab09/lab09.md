...
1. Каков IP-адрес вашего хоста? Каков IP-адрес хоста назначения?
   ![](Screenshot_20240530_014039.png)
   - IP-адрес хоста отправки запроса - 192.168.31.244
   - IP-адрес хоста назначения - 166.111.4.100
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - С помощью протокола ICMP не передаются данные приложений, и каждый запрос обрабатывается на уровне хоста. Поэтому порт (который выполняет функцию идентификации приложения) не используется в протоколе ICMP.
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля 
   контрольной суммы, порядкового номера и идентификатора?
   - Тип 8 (echo-запрос), кодовый номер - 0
   - Контрольная сумма, идентификатор, порядковый номер и метка времени
...
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   ![](Screenshot_20240530_015136.png)
   - ICMP тип - 0 (echo-ответ), кодовый номер - 0
   - Контрольная сумма, идентификатор, порядковый номер и метка времени
   - По 2 байта на контрольную сумму, идентификатор и порядковый номер
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   ![](Screenshot_20240530_020104.png)
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   ![](Screenshot_20240530_020410.png)
   - Поле unused и информация и заголовок соответствующего запроса
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   ![](Screenshot_20240530_021417.png)
   - Другой тип пакета (0 вместо 11)
   - Содержатся данные из ICMP-запроса
   - Эти пакеты пришли непосредственно от хоста, который был передан аргументом в traceroute (то есть, дошли до конечной точки запроса)
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   ![](Screenshot_20240530_021755.png)
   - Можно определить, что максимальный рост задержки происходит между маршрутизаторами 12 и 14
   - Оба IP соответствуют хостам из Гонконга
