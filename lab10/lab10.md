...
1. Выберите первое ICMP-сообщение эхо-запроса, отправленное вашим компьютером, и
   ...
   - ![](Screenshot_20240429_222639.png)
     192.168.31.244
...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   - 20 (56 в IP-пакете, 36 в UDP-пакете)
   - 28
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - ![](Screenshot_20240430_001956.png)
   ...
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не изменяются - Version, Header Length, Differentiated Services Field, Total Length, Flags, Fragment Offset, Protocol, Source/Destination Address
      - Изменяются - Identification, Header Checksum, Time to Live
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - ![](Screenshot_20240430_003740.png)
     ![](Screenshot_20240430_003451.png)
     Нет, TTL выставляется равным 64, Identification также не совпадает
7. Найдите серии откликов ICMP, в которых содержатся сообщения об истечении
   ...
   - Identification - некоторое значение, определённое маршрутизатором и отличающееся на 1 для последовательных ответов от одного маршрутизатора
   - TTL - 64 для всех ответов
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - Среди ответов нет ни одного, содержащего эхо-запрос. Фрагментированы были запросы: каждый из них разбит на 3 фрагмента (~1500, ~1500 и ~500 байт).
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - ![](Screenshot_20240430_010927.png)
        Header Checksum и Fragment Offset
